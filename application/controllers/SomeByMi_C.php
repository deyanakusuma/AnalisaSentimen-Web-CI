...
class SomeByMi_C extends CI_Controller {
	public function __construct(){
        parent::__construct();
        $this->load->model('SomeByMi_m');
        $this->load->helper('url');
    }
	public function index()
    {
        $data['opinis']  = $this->SomeByMi_m->get_all();
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=14 AND result='positif'"); 
        $data['positif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=14 AND result='negatif'"); 
        $data['negatif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=14 AND result='netral'"); 
        $data['netral'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);

        $this->load->view('somebymi', $data);
    }
    public function edit($id)
    {
        $id_opini = $this->SomeByMi_m->get_by_id($id);
        ...
        $file['active_page']="SomeByMi_C";
        $this->load->view('somebymi_detail', $file);
    }
}
