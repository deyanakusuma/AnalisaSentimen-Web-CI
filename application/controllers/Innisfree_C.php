...
class Innisfree_C extends CI_Controller {
	public function __construct(){
        parent::__construct();
        $this->load->model('Innisfree_m');
        $this->load->helper('url');
    }
	public function index()
    {
        $data['opinis']  = $this->Innisfree_m->get_all();
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=9 AND result='positif'"); 
        $data['positif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=9 AND result='negatif'"); 
        $data['negatif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=9 AND result='netral'"); 
        $data['netral'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);

        $this->load->view('innisfree', $data);
    }
    public function edit($id)
    {
        $id_opini = $this->Innisfree_m->get_by_id($id);
        ...
        if ($id_opini) {
            ...
            redirect('Innisfree_C');
        }
        $file['active_page']="Innisfree_C";
        $this->load->view('innisfree_detail', $file);
    }
}
