...
class Marcks_C extends CI_Controller {
	public function __construct(){
        parent::__construct();
        $this->load->model('Marcks_m');
        $this->load->helper('url');
    }
	public function index()
    {
        $data['opinis']  = $this->Marcks_m->get_all();
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=7 AND result='positif'"); 
        $data['positif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=7 AND result='negatif'"); 
        $data['negatif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=7 AND result='netral'"); 
        $data['netral'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);

        $this->load->view('marcks', $data);
    }
    public function edit($id)
    {
        $id_opini = $this->Marcks_m->get_by_id($id);
        ...
        $file['active_page']="Marcks_C";
        $this->load->view('marcks_detail', $file);
    }
}
