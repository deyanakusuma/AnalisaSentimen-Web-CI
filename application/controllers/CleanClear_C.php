...
class CleanClear_C extends CI_Controller {
	public function __construct(){
        parent::__construct();
        $this->load->model('CleanClear_m');
        $this->load->helper('url');
    }
	public function index()
    {
        $data['opinis']  = $this->CleanClear_m->get_all();
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=4 AND result='positif'"); 
        $data['positif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=4 AND result='negatif'"); 
        $data['negatif'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);
        $query = $this->db->query("SELECT COUNT(*) as count FROM hasil_sentiment WHERE id_brand=4 AND result='netral'"); 
        $data['netral'] = json_encode(array_column($query->result(), 'count'),JSON_NUMERIC_CHECK);

        $this->load->view('cleanclear', $data);
    }
    public function edit($id)
    {
        $id_opini = $this->CleanClear_m->get_by_id($id);
        ...
        if ($id_opini) {
            ...
            redirect('CleanClear_C');
        }
        $file['active_page']="CleanClear_C";
        $this->load->view('cleanclear_detail', $file);
    }
}
